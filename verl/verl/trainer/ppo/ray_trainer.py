...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        sample_scores = []
        sample_turns = []
        sample_response_lengths = torch.tensor([])
        ...
        for test_data in self.val_dataloader:
            ...
            sample_response_lengths = torch.cat([sample_response_lengths, response_info["response_length"]])
        ...
        metric_dict["val-len/response_length/mean"] = sample_response_lengths.mean().detach().item()
        metric_dict["val-len/response_length/max"] = sample_response_lengths.max().detach().item()
        metric_dict["val-len/response_length/min"] = sample_response_lengths.min().detach().item()
        metric_dict["val-len/response_length/clip_ratio"] = sample_response_lengths.eq(self.config.data.max_response_length).float().mean().detach().item()
    ...
    def _get_passrate_metrics(self, batch: DataProto):
